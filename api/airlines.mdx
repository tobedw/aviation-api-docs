...
Prepend `https://airlines-api.logostream.dev` and append `?key=YOUR_API_KEY` to build the full URL:

```javascript
const logo = `https://airlines-api.logostream.dev${airline.logo.icon}?key=YOUR_API_KEY`
```
