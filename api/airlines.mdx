...
Prepend `https://airline-api.logostream.dev` and append `?key=YOUR_API_KEY` to build the full URL:

```javascript
const logo = `https://airline-api.logostream.dev${airline.logo.icon}?key=YOUR_API_KEY`
```
